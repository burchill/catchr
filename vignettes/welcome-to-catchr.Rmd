...
Let's hit you with some imaginary scenarios here. Suppose I want to run some code, and I want to catch any warnings it produces, "hide" them so they don't appear, and instead just store them so I can do something else with them later on.  All without making my code halt or start over.
...
Particularly useful for when you're working in a different window and waiting for a job to finish / do something unexpected, `catchr` lets you play short sounds whenever conditions are raised.  This functionality requires the [`beepr`](https://cran.r-project.org/web/packages/beepr/index.html) package, which I've found absolutely worth downloading.

For example, you can have it alert you when a condition is raised via sound, display it, and keep going:
